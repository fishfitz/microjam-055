<script setup>
const [gameWidth, gameHeight] = [3840, 2160]
const { width, height } = useWindowSize()
const scale = computed(() => {
  const factor = Math.min(width.value / gameWidth, height.value / gameHeight)
...
<template>
  <div class="main-container bg-black h-screen w-screen fixed top-0 left-0 flex justify-center items-center">
    ...
    <div :style="{ transform: scale }">
      ...
      <NuxtPage :style="{ height: `${gameHeight}px`, width: `${gameWidth}px` }" class="bg-black overflow-hidden" />
    </div>
  </div>
</template>
...
.noise {
  ...
  animation: grain 1.5s steps(6) infinite alternate;
  pointer-events: none;
  z-index: 99;
}

.focus {
  background: radial-gradient(circle, rgba(2,0,36,0) 45%, rgba(0,0,0,1) 100%);;
  mix-blend-mode: hard-light;
  pointer-events: none;
}
